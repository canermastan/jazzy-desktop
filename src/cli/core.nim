...
import std/[os, osproc, strutils, times, tables, strformat, streams, terminal]
import scaffold
...
proc showHelp() =
  ...
  echo "Usage:"
  echo "  jazzyd new <name>  Create a new Jazzy Desktop project"
...
when isMainModule:
  ...
  of "new":
    if args.len < 2:
      echo "❌ Usage: jazzyd new <project-name>"
      quit(1)
    runNew(args[1])
  of "dev":
    runDev(platform)
